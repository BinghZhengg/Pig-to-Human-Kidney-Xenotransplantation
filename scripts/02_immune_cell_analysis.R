...
Immune <- FindClusters(Immune, resolution = 0.1) # 0.1
...
# Figure 2 B
DimPlot(Immune) + NoLegend() + theme(axis.title = element_blank())

# save Immune Cell Analysis
saveRDS(Immune, "data/Immune_subclustered.rds")
Immune <- readRDS("data/Immune_subclustered.rds")
...
# Figure 2 C
heatmap_markers <- ggplot(bulk_markers_melt, 
									aes(x = variable, y = factor(hp_ortho_genes, 
																		  levels = rev(markers)), 
																		  fill = value)) +
										geom_tile() +
...
# Figure 2 F
heatmap_ifng_inducible <- ggplot(bulk_ifng_inducible_melt, 
								  aes(x = variable, y = factor(hp_ortho_genes, 
