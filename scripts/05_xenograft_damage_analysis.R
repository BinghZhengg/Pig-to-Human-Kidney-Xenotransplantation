...
deg.1.df <- FindMarkers(PTC_1, assay = "RNA", ident.1 = "kidney_1st_treated") %>% arrange(desc(avg_log2FC))

Idents(PTC_2) <- PTC_2$Condition

deg.2.df <- FindMarkers(PTC_2, assay = "RNA", ident.1 = "kidney_2nd_treated") %>% arrange(desc(avg_log2FC))
# set thresholds 
fold_cutoff = 0.5
pvalue_cutoff = 0.01
...
volcanoPlot <- function(results.df, title = "Volcano Plot of Gene Expression", manual_labels = NULL) {

	# Create factor denoting differential expression status
	stats.df <- results.df %>%
		mutate(diffexpressed = ifelse(geneIDs %in% manual_labels, "Labelled", "Not labelled"))
	...
	plot <- plot +
		geom_label_repel(data = labelled_genes, aes(label = geneIDs), 
				 box.padding = 0.4, 
				 point.padding = 0.2, 
				 max.overlaps = 50)
...
# Figure 4 A
volcanoPlot(plot.deg1, manual_labels = c("SPP1",
					 "GPNMB",
					 "S100A6",
					 "PCLAF", # Proliferating cells marker
					 "HMGN2", # Proliferating cells marker
					 "HMGB2", # Proliferating cells marker
					 "STMN1", # Proliferating cells marker
					 cc.genes$s.genes, # Unsupervised proliferation markers
					 cc.genes$g2m.genes)) + NoLegend()
volcanoPlot(plot.deg2, manual_labels = c("SPP1",
					 "GPNMB",
					 "S100A6",
					 "PCLAF", # Proliferating cells marker
					 "HMGN2", # Proliferating cells marker
...
nephrons <- subset(kidneys, subset = cell_types %in% c("PT",
